...
public class UpgradeUi : MonoBehaviour
{
    ...
    private List<string> _explanationUpgradeModules = new List<string>
    {
        "마지막 한 발의\n피해가 증가합니다",
        "섬광탄의 피해량과\n밀어내는 거리가 증가합니다",
        "헤드샷의 확률이 증가합니다",
    ...
    public void SetModule(int[] modules, int disposableModule)
    {
        ...
        for (int i = 0; i < modules.Length; i++)
        {
            ...
            if (UpgradeManager.upgradeManager.UpgradeModule[modules[i]] > 0)
                Explanations[i].text = _explanationUpgradeModules[modules[i]];
            else
    ...
    public void UpgradeModules(int number)
    {
